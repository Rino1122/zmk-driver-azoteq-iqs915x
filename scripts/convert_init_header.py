...
def parse_header(filepath: str) -> list[int]:
    ...
    """
    values = []
    define_pattern = re.compile(r"^\s*#define\s+\w+\s+(0x[0-9A-Fa-f]+)\s*$")
    ...
    with open(filepath, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            if match:
                ...
                if val > 0xFF:
                    print(
                        f"Warning: value {match.group(1)} exceeds 0xFF, "
                        f"truncating to lower byte",
                        file=sys.stderr,
                    )
                    val = val & 0xFF
                values.append(val)
...
def validate_values(values: list[int]) -> bool:
    ...
    if len(values) != EXPECTED_TOTAL:
        print(
            f"Error: Expected {EXPECTED_TOTAL} bytes, "
            f"but extracted {len(values)} bytes.",
            file=sys.stderr,
        )
        print(
            f"  Expected: {EXPECTED_MAIN_SIZE} (main) + "
...
def format_c_array_output(values: list[int]) -> str:
    """バイト値を配列ヘッダー形式に変換する。"""
    lines = []
    lines.append("/* Auto-generated from Azoteq GUI header file */")
    lines.append("/* Generated array header for IQS915x built-in init-data */")
    lines.append("#ifndef IQS915X_INIT_DATA_BRETAGNE_ARRAY_H")
    lines.append("#define IQS915X_INIT_DATA_BRETAGNE_ARRAY_H")
    lines.append("")
    lines.append("#include <stdint.h>")
    lines.append("")
    lines.append("static const uint8_t iqs915x_init_data_bretagne[] = {")
    ...
    lines.append("};")
    lines.append("")
    lines.append("#endif /* IQS915X_INIT_DATA_BRETAGNE_ARRAY_H */")
...
def main():
    ...
    print(f"Extracted {len(values)} byte values from {filepath}", file=sys.stderr)
    ...
    if not validate_values(values):
        ...
        print(
            f"\nSection breakdown (total expected: {total_expected}):", file=sys.stderr
        )
        for name, addr, size in SECTIONS:
